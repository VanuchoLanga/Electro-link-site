...
/* ══ CREDENCIAIS ══
   Para alterar: modifica os valores abaixo.
   Utilizador: admin@electrolink.co.mz
   Password:   Electro@2026
*/
const CREDENTIALS = {
  email:    'admin@electrolink.co.mz',
  password: 'Electro@2026',
};
...
function checkSession() {
  ...
  // se não há sessão, mostra o ecrã de login (já visível por defeito)
}
...
window.salvarCliente = function() {
  ...
  const db = getDB();
  db.clientes.push({ id:genId(), nome, tel,
...
window.editCliente = function(id) {
  const db = getDB(); const c = db.clientes.find(x=>x.id===id); if(!c) return;
  ...
  db.clientes = db.clientes.filter(x=>x.id!==id); saveDB(db);
  openDrawer('drawer-cliente'); renderAll();
};

window.deleteCliente = function(id) {
  if(!confirm('Apagar este cliente?')) return;
  const db=getDB(); db.clientes=db.clientes.filter(c=>c.id!==id); saveDB(db); renderAll();
};

window.filterClientes = function(q) { renderClientes(q.toLowerCase()); };

function renderClientes(q='') {
  const db=getDB(); const tbody=document.getElementById('clientes-tbody');
  let items=q?db.clientes.filter(c=>c.nome.toLowerCase().includes(q)||c.tel.includes(q)||(c.local||'').toLowerCase().includes(q)):db.clientes;
  ...
  tbody.innerHTML=items.map(c=>`
    <tr>
      ...
      <td class="muted">${c.tel}${c.email?'<br><small style="font-size:11px;">'+c.email+'</small>':''}</td>
      <td>${c.servico}</td>
...
window.salvarVaga = function() {
  ...
  const db=getDB();
  db.vagas.push({ id:genId(), titulo, area,
  ...
  saveDB(db); closeDrawer('drawer-vaga');
  ['vaga-titulo','vaga-local','vaga-prazo','vaga-num','vaga-desc'].forEach(i=>document.getElementById(i).value='');
  document.getElementById('vaga-area').value=''; renderAll();
};

window.fecharVaga = function(id) {
  if(!confirm('Encerrar esta vaga?')) return;
  const db=getDB(); const v=db.vagas.find(x=>x.id===id); if(v) v.estado='encerrada';
  saveDB(db); renderAll();
};

window.deleteVaga = function(id) {
  if(!confirm('Apagar esta vaga?')) return;
  const db=getDB(); db.vagas=db.vagas.filter(v=>v.id!==id); saveDB(db); renderAll();
};

function renderVagas() {
  const db=getDB(); const container=document.getElementById('vagas-list');
  if(!db.vagas.length){ container.innerHTML=`<div class="empty-state" style="background:#fff;border:1px solid var(--gray-200);border-radius:var(--radius);">
  ...
  container.innerHTML=db.vagas.map(v=>{
    ...
    return `<div class="vaga-card">
      <div>
        ...
        <h3>${v.titulo}</h3>
        <p style="font-size:13px;color:var(--gray-600);margin-top:5px;">${v.desc.length>110?v.desc.slice(0,110)+'…':v.desc}</p>
        <div class="vaga-meta">
...
window.updateEstado = function(id, estado) {
  const db=getDB(); const c=db.candidaturas.find(x=>x.id===id); if(c) c.estado=estado;
  saveDB(db); closeDrawer('drawer-cand-detail'); renderAll();
};

window.verCandidatura = function(id) {
  const db=getDB(); const c=db.candidaturas.find(x=>x.id===id); if(!c) return;
...
function badgeClass(e){ return {pendente:'b-gold','em-analise':'b-navy',aprovado:'b-green',rejeitado:'b-red'}[e]||'b-gray'; }
...
function renderCandidaturas() {
  const db=getDB(); let items=db.candidaturas;
  ...
  if(!items.length){ tbody.innerHTML=emptyRow(6,_cfq||_cfs?'Nenhum resultado':'Sem candidaturas','As candidaturas submetidas aparecerão aqui.'); return; }
...
function renderAll() {
  renderClientes(); renderVagas(); renderCandidaturas(); updateStats();
}
